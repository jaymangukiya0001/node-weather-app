...
const forecast=(lat,log,callback)=>{
    ...
    request({url:url,json:true},(error,res)=>{
        if(error){
            ...
            callback(undefined,{
                data:`${res.body.current.weather_descriptions[0]} it is currently  ${res.body.current.temperature} but it feel like ${res.body.current.feelslike} degrees out The humadity is ${res.body.current.humidity}%`,
            })
        }
    })
}
